...
if [[ ! -f "${NODE_HOME}/config/config.toml" || ! -f "${NODE_HOME}/config/genesis.json" || ! -f "${NODE_HOME}/data/priv_validator_state.json" ]]; then
  if [[ "${DOWNLOAD_CONFIG_IF_MISSING}" == "1" ]]; then
    ...
    rm -rf "./local-test-hoodi/.config-extract" "./local-test-hoodi/hoodi-data"
    ...
    echo "Downloading hoodi config bundle..."
    curl -fL "${HOODI_CONFIG_ZIP_URL}" -o "${CONFIG_ZIP_PATH}"
    ...
    elif [[ -f "${temp_extract_dir}/hoodi-data/node-data/config/config.toml" && -f "${temp_extract_dir}/hoodi-data/node-data/config/genesis.json" ]]; then
      bundle_root="${temp_extract_dir}/hoodi-data"
    ...
    if [[ -z "${bundle_root}" ]]; then
      ...
      echo "Checked bundle roots: ${temp_extract_dir}/data, ${temp_extract_dir}/hoodi-data, ${temp_extract_dir}"
      exit 1
    fi
    ...
    echo "Config prepared at ${NODE_HOME} from ${HOODI_CONFIG_ZIP_URL}"
  else
...
if [[ -f "${NODE_LOG_FILE}" ]] && grep -q "wrong block number" "${NODE_LOG_FILE}"; then
  ...
  if [[ "${AUTO_RESET_ON_WRONG_BLOCK}" == "1" ]]; then
    ...
    echo "If replay fails again, run: ./local-test-hoodi/reset.sh --yes"
  fi
fi
