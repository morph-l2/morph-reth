...
# Morphnode configuration (binary is in ../morph/node, data is in local-test-hoodi)
: "${MORPHNODE_BIN:=../morph/node/build/bin/morphnode}"
: "${NODE_HOME:=./local-test-hoodi/node-data}"
: "${JWT_SECRET:=./local-test-hoodi/jwt-secret.txt}"
: "${NODE_LOG_FILE:=./local-test-hoodi/node.log}"
...
: "${HOODI_CONFIG_ZIP_URL:=https://raw.githubusercontent.com/morph-l2/run-morph-node/main/hoodi/data.zip}"
: "${CONFIG_ZIP_PATH:=./local-test-hoodi/hoodi-data.zip}"
: "${KEEP_CONFIG_ARTIFACTS:=0}"
: "${AUTO_RESET_ON_WRONG_BLOCK:=0}"

# Morph Geth configuration
: "${GETH_BIN:=../morph/go-ethereum/build/bin/geth}"
: "${GETH_DATA_DIR:=./local-test-hoodi/geth-data}"
: "${GETH_LOG_FILE:=./local-test-hoodi/geth.log}"

# Morph-Reth configuration
: "${RETH_BIN:=./target/release/morph-reth}"
: "${RETH_DATA_DIR:=./local-test-hoodi/reth-data}"
: "${RETH_LOG_FILE:=./local-test-hoodi/reth.log}"
