...
use clap::Parser;
use morph_chainspec::{MORPH_BASE_FEE, MorphChainSpec, MorphChainSpecParser};
...
fn morph_default_suggested_fee() -> U256 {
    U256::from_limbs([MORPH_BASE_FEE, 0, 0, 0])
}
...
mod tests {
    use super::*;
    use reth_ethereum_cli::Commands;

    #[test]
    fn default_gpo_suggested_fee_matches_morph_geth() {
        let mut cli =
            Cli::<MorphChainSpecParser, MorphArgs, DefaultRpcModuleValidator>::parse_from([
                "morph-reth",
                "node",
            ]);
        apply_morph_cli_defaults(&mut cli);
        ...
        assert_eq!(
            command.rpc.gas_price_oracle.default_suggested_fee,
            Some(morph_default_suggested_fee())
        );
    }

    #[test]
    fn explicit_gpo_suggested_fee_overrides_morph_default() {
        let mut cli =
            Cli::<MorphChainSpecParser, MorphArgs, DefaultRpcModuleValidator>::parse_from([
                "morph-reth",
                "node",
        ...
        assert_eq!(
            command.rpc.gas_price_oracle.default_suggested_fee,
            Some(U256::from_limbs([123, 0, 0, 0]))
        );
    }
}
