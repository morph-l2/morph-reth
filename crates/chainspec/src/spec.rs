...
mod tests {
    ...
    #[test]
    fn test_onyx_activation_from_genesis() {
        let genesis_json = json!({
            "config": {
                "chainId": 1337,
                "homesteadBlock": 0,
        ...
        assert_eq!(
            chainspec.fork(MorphHardfork::Onyx),
        ...
        // Onyx must be reported as the latest fork once active, and must not
        // shadow Jade before its own activation.
        assert_eq!(chainspec.morph_hardfork_at(1, 6000), MorphHardfork::Jade);
        assert_eq!(chainspec.morph_hardfork_at(1, 7000), MorphHardfork::Onyx);
    }

    #[test]
    fn test_onyx_absent_from_genesis_never_activates() {
        // The bundled mainnet/hoodi chainspecs are scheduled through Jade only.
        let chainspec = MorphChainSpec::from(create_test_genesis());
        assert!(!chainspec.is_onyx_active_at_timestamp(0));
        assert!(!chainspec.is_onyx_active_at_timestamp(u64::MAX));
    }
