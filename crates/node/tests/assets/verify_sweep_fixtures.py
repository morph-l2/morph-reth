#!/usr/bin/env python3
"""Verify embedded sweep test-fixture runtimes against their Solidity source.
...
`forge` is required (solc 0.8.30 is fetched via svm on first use). It recompiles
each contract in `SweepFixtures.sol` and checks the embedded Rust runtime
constant in `tests/it/sweep.rs` matches both the manifest SHA-256 and a fresh
`forge inspect deployedBytecode`.
"""
