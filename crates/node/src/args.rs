...
pub struct MorphArgs {
    ...
    /// Use reth's `--builder.deadline` as the Morph payload builder time limit.
    ///
    /// The Morph builder stops packing after its own 1 s budget so that
    /// `engine_assembleL2Block` answers within a block interval. Synthetic
    /// benchmarks that assemble very large blocks opt in to the longer reth
    /// deadline instead. Not intended for production sequencers.
    #[arg(long = "morph.builder-use-reth-deadline", default_value_t = false)]
    pub builder_use_reth_deadline: bool,
...
impl Default for MorphArgs {
    fn default() -> Self {
        Self {
            ...
            builder_use_reth_deadline: false,
...
mod tests {
    ...
    fn test_default_args() {
        ...
        assert!(!args.builder_use_reth_deadline);
    ...
    #[test]
    fn builder_reth_deadline_is_opt_in() {
        let default = CommandParser::<MorphArgs>::parse_from(["test"]).args;
        assert!(!default.builder_use_reth_deadline);

        let benchmark =
            CommandParser::<MorphArgs>::parse_from(["test", "--morph.builder-use-reth-deadline"])
                .args;
        assert!(benchmark.builder_use_reth_deadline);
    }

    #[test]
    fn benchmark_payload_bypass_flag_is_not_supported() {
        // The former benchmark-only bypass is gone: the consensus limit comes from genesis.
        assert!(
