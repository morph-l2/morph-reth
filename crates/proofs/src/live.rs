...
use reth_provider::{
    DatabaseProviderFactory, HashedPostStateProvider, StateProviderFactory, StateRootProvider,
};
...
use crate::{
    BlockStateDiff, MorphProofsStorage, MorphProofsStorageError, MorphProofsStore,
    api::{MorphProofsBatchSession, MorphProofsBatchStore, OperationDurations, WriteCounts},
    batch_provider::MorphProofsBatchStateProviderRef,
    metrics::ProofHistoryMetrics,
    provider::MorphProofsStateProviderRef,
};
...
where
    Evm: ConfigureEvm,
    Provider: DatabaseProviderFactory + StateProviderFactory,
{
    evm_config: Evm,
    provider: Provider,
    storage: &'tx MorphProofsStorage<PreimageStore>,
}
...
where
    Evm: ConfigureEvm,
    Provider: DatabaseProviderFactory + StateProviderFactory,
    Store: 'tx + MorphProofsStore + Clone + 'static,
{
    fn record_storage_metrics(
        operation_durations: &OperationDurations,
        write_counts: Option<&WriteCounts>,
    ) {
        ProofHistoryMetrics::record_block(operation_durations, write_counts);
    }
    ...
    pub fn execute_and_store_block_updates(
        ...
        let (Some((earliest, _)), Some((latest, _))) = (
            ...
            return Err(MorphProofsStorageError::NoBlocksFound);
        };

        let parent_block_number = block.number() - 1;
        if parent_block_number < earliest {
            return Err(MorphProofsStorageError::UnknownParent);
        }

        if parent_block_number > latest {
            return Err(MorphProofsStorageError::MissingParentBlock {
        ...
        let state_provider = MorphProofsStateProviderRef::new(
        ...
        if state_root != block.state_root() {
            return Err(MorphProofsStorageError::StateRootMismatch {
                block_number: block.number(),
    ...
    pub fn store_block_updates(
        ...
        let storage_result = match self.storage.store_trie_updates(
            block,
            BlockStateDiff {
                sorted_trie_updates,
                sorted_post_state,
    ...
    pub fn unwind_and_store_block_updates(
        ...
        self.storage
            .replace_updates(latest_common_block, block_trie_updates)?;
        let write_duration = start.elapsed();
        operation_durations.total_duration_seconds = write_duration;
        operation_durations.write_duration_seconds = write_duration;
    ...
    pub fn unwind_history(&self, to: BlockWithParent) -> Result<(), MorphProofsStorageError> {
        if let Err(error) = self.storage.unwind_history(to) {
            ProofHistoryMetrics::record_unwind_error();
            return Err(error);
        }
        if let Some((earliest, _)) = self.storage.get_earliest_block_number()?
            && let Some((latest, _)) = self.storage.get_latest_block_number()?
        {
            ProofHistoryMetrics::set_window(earliest, latest);
        }
        Ok(())
    }
}
...
where
    Evm: ConfigureEvm,
    Provider: DatabaseProviderFactory + StateProviderFactory,
    Store: 'tx + MorphProofsBatchStore + Clone + 'static,
{
    ...
    pub fn execute_and_store_batch(
        ...
        self.storage.with_batch_session(|session| {
            let (Some((earliest, _)), Some((_, _))) = (
                session.get_earliest_block_number()?,
                session.get_latest_block_number()?,
            ) else {
                return Err(MorphProofsStorageError::NoBlocksFound);
            };
        ...
        ProofHistoryMetrics::record_block(
            &OperationDurations {
        ...
        if let Some((earliest, _)) = self.storage.get_earliest_block_number()? {
            ProofHistoryMetrics::set_window(earliest, last_block_number);
        }
    ...
    fn execute_one_in_session<S>(
        ...
        block: &RecoveredBlock<BlockTy<Evm::Primitives>>,
        earliest: u64,
    ) -> Result<WriteCounts, MorphProofsStorageError>
    where
        S: MorphProofsBatchSession,
    {
        let latest_in_session = session
            .get_latest_block_number()?
            .ok_or(MorphProofsStorageError::NoBlocksFound)?
            .0;
        ...
        let state_provider = MorphProofsBatchStateProviderRef::new(
            self.provider.latest()?,
            session,
            parent_block_number,
        );
