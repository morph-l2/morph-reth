...
use crate::{
    MorphProofsStorageError, MorphProofsStore,
    api::{InitialStateAnchor, InitialStateStatus, MorphProofsInitialStateStore},
...
impl<Tx: DbTx + Sync, S: MorphProofsStore + MorphProofsInitialStateStore + Send>
    InitializationJob<Tx, S>
{
    ...
    >(
        ...
        log_threshold: usize,
    ) -> Result<u64, MorphProofsStorageError> {
    ...
    fn initialize_trie(&self, anchor: InitialStateAnchor) -> Result<(), MorphProofsStorageError> {
    ...
    fn validate_anchor_block(
        ...
        if block.number != best_number || block.hash != best_hash {
            return Err(MorphProofsStorageError::InitializeStorageInconsistentState);
        }
    ...
    pub fn run(&self, best_number: u64, best_hash: B256) -> Result<(), MorphProofsStorageError> {
        ...
        ($mod_name:ident, $storage:ident) => {
            mod $mod_name {
                ...
                fn test_initialize_hashed_storage() {
                    ...
                    assert_eq!(
                        found[0],
                        (storage_entries[0].1.key, storage_entries[0].1.value)
                    );
                    assert_eq!(
                        found[1],
                        (storage_entries[1].1.key, storage_entries[1].1.value)
                    );
                    ...
                    assert_eq!(
                        found[0],
                        (storage_entries[2].1.key, storage_entries[2].1.value)
                    );
                }
