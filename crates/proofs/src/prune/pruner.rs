...
use crate::{
    MorphProofsStorage, MorphProofsStore, ProofHistoryMetrics,
    prune::{
        error::{MorphProofStoragePrunerResult, PrunerError, PrunerOutput},
...
#[derive(Debug)]
pub struct MorphProofStoragePruner<P, H> {
    /// Database provider for the prune
    provider: MorphProofsStorage<P>,
...
impl<P, H> MorphProofStoragePruner<P, H> {
    /// Create a new pruner.
    pub const fn new(
        provider: MorphProofsStorage<P>,
        block_hash_reader: H,
        ...
        assert!(
            prune_batch_size > 0,
            "prune batch size must be greater than zero"
        );
        Self {
            provider,
            block_hash_reader,
    ...
    H: BlockHashReader,
{
    ...
    /// Runs the pruner and returns failures to command-style callers.
    pub fn try_run(&self) -> MorphProofStoragePrunerResult {
        let result = self.run_inner();
        match &result {
            Ok(_) => {
                if let Ok(Some((earliest, _))) = self.provider.get_earliest_block_number()
                    && let Ok(Some((latest, _))) = self.provider.get_latest_block_number()
                {
                    ProofHistoryMetrics::set_window(earliest, latest);
                }
            }
            Err(_) => ProofHistoryMetrics::record_prune_error(),
        }
        result
    }
}
        ...
        ($mod_name:ident, $storage:ident) => {
            mod $mod_name {
                ...
                async fn run_inner_and_and_verify_updated_state() {
                    ...
                    let store: MorphProofsStorage<Arc<$storage>> =
                        MorphProofsStorage::from(Arc::new($storage::new(dir.path()).expect("env")));
                    ...
                    let pruner =
                        MorphProofStoragePruner::new(store.clone(), block_hash_reader, 1, 1000);
                    let out = pruner.run_inner().expect("pruner ok");
                    assert_eq!(out.start_block, 0);
                    assert_eq!(
                        out.end_block, 4,
                        "pruned up to 4 (inclusive); new earliest is 4"
                    );
                    ...
                    let mut stor_trie_cur = store
                        .storage_trie_cursor(stor_addr, 4)
                        .expect("stor trie cur");
                ...
                async fn run_inner_where_latest_block_is_none() {
                    ...
                    let pruner = MorphProofStoragePruner::new(store, block_hash_reader, 10, 1000);
                    let out = pruner.run_inner().expect("ok");
                    assert_eq!(
                        out,
                        PrunerOutput::default(),
                ...
                async fn run_inner_earliest_none_real_db() {
                    ...
                    let block_hash_reader = MockBlockHashReader::new();
                    let pruner = MorphProofStoragePruner::new(store, block_hash_reader, 1, 1000);
                    let out = pruner.run_inner().expect("ok");
                    assert_eq!(
                        out,
                        PrunerOutput::default(),
                ...
                async fn run_inner_interval_too_small_real_db() {
                    ...
                    let h4 = b256(4);
                    store
                        .set_earliest_block_number(earliest_num, h4)
                        .expect("set earliest");

                    // Set latest=5 by storing block 5
                    let b5 = block(5, h4);
                    store
                        .store_trie_updates(b5, BlockStateDiff::default())
                        .expect("store b5");
