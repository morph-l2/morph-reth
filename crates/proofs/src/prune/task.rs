...
#[derive(Debug)]
pub struct MorphProofStoragePrunerTask<P, H> {
    pruner: MorphProofStoragePruner<P, H>,
    retention_blocks: u64,
    task_run_interval: Duration,
}

impl<P, H> MorphProofStoragePrunerTask<P, H>
where
    P: MorphProofsStore + Send + Sync + 'static,
    H: BlockHashReader + Send + Sync + 'static,
{
    /// Initialize a new [`MorphProofStoragePrunerTask`]
    pub const fn new(
        ...
        let pruner =
            MorphProofStoragePruner::new(provider, hash_reader, retention_blocks, PRUNE_BATCH_SIZE);
        Self {
            pruner,
            retention_blocks,
            task_run_interval,
