...
use crate::{
    BlockStateDiff, MorphProofsStorageError, MorphProofsStorageResult,
    api::{MorphProofsBatchSession, WriteCounts},
    db::{
...
impl<'tx> MdbxBatchSession<'tx> {
    ...
    pub(crate) fn commit(mut self) -> MorphProofsStorageResult<()> {
    ...
    fn tx_ref(&self) -> MorphProofsStorageResult<&<DatabaseEnv as Database>::TXMut> {
        self.tx
            .as_ref()
            .ok_or(MorphProofsStorageError::BatchSessionClosed)
    }

    fn dup_cursor<T: Table + DupSort>(&self) -> MorphProofsStorageResult<DupRw<'_, T>> {
        Ok(self.tx_ref()?.cursor_dup_read::<T>()?)
    }
}

impl MorphProofsBatchSession for MdbxBatchSession<'_> {
    ...
    fn get_earliest_block_number(&self) -> MorphProofsStorageResult<Option<(u64, B256)>> {
        self.storage
            .inner_get_earliest_block_number_hash(self.tx_ref()?)
    }

    fn get_latest_block_number(&self) -> MorphProofsStorageResult<Option<(u64, B256)>> {
        self.storage
            .inner_get_latest_block_number_hash(self.tx_ref()?)
    }
    ...
    fn account_hashed_cursor(
        ...
        Ok(MdbxAccountCursor::new(
            self.dup_cursor::<HashedAccountHistory>()?,
            max_block_number,
        ))
    }
    ...
    fn store_trie_updates(
        ...
        self.storage
            .store_trie_updates_append_only(self.tx_ref()?, block_ref, block_state_diff)
    }
}
