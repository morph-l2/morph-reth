    ...
    Cursor: DbCursorRO<T> + DbDupCursorRO<T>,
{
    ...
    fn is_positioned(&mut self) -> MorphProofsStorageResult<bool> {
    ...
    fn latest_version_for_key(
        &mut self,
        key: T::Key,
    ) -> MorphProofsStorageResult<Option<(T::Key, T::Value)>> {
    ...
    fn seek_exact(&mut self, key: T::Key) -> MorphProofsStorageResult<Option<(T::Key, V)>> {
    ...
    fn next_live_from(
        &mut self,
        mut first_key: T::Key,
    ) -> MorphProofsStorageResult<Option<(T::Key, V)>> {
        loop {
    ...
    fn seek(&mut self, start_key: T::Key) -> MorphProofsStorageResult<Option<(T::Key, V)>> {
    ...
    fn next(&mut self) -> MorphProofsStorageResult<Option<(T::Key, V)>>
    where
        T::Key: Default,
    {
...
> MdbxTrieCursor<T, Cursor>
{
    ...
    pub const fn new(cursor: Cursor, max_block_number: u64, hashed_address: Option<B256>) -> Self {
        Self {
            inner: BlockNumberVersionedCursor::new(cursor, max_block_number),
            hashed_address,
        }
    }
}
    ...
    Cursor: DbCursorRO<HashedStorageHistory> + DbDupCursorRO<HashedStorageHistory> + Send + Sync,
{
    ...
    pub const fn new(cursor: Cursor, block_number: u64, hashed_address: B256) -> Self {
        Self {
            inner: BlockNumberVersionedCursor::new(cursor, block_number),
            hashed_address,
        }
    }
}
...
mod tests {
    ...
    fn append_account_trie(
        ...
        let mut c = wtx
            .cursor_dup_write::<AccountTrieHistory>()
            .expect("dup write cursor");
        let vv = VersionedValue {
            block_number: block,
            value: MaybeDeleted(val),
        };
    ...
    fn append_storage_trie(
        ...
        let mut c = wtx
            .cursor_dup_write::<StorageTrieHistory>()
            .expect("dup write cursor");
        let key = StorageTrieKey::new(address, StoredNibbles(path));
        let vv = VersionedValue {
            block_number: block,
    ...
    fn append_hashed_storage(
        ...
        let mut c = wtx
            .cursor_dup_write::<HashedStorageHistory>()
            .expect("dup write");
        let key = HashedStorageKey::new(addr, slot);
        let vv = VersionedValue {
            block_number: block,
            value: MaybeDeleted(val.map(StorageValue)),
        };
        c.append_dup(key, vv).expect("append dup");
    }
    ...
    fn append_hashed_account(
        ...
        let mut c = wtx
            .cursor_dup_write::<HashedAccountHistory>()
            .expect("dup write");
        let vv = VersionedValue {
            block_number: block,
            value: MaybeDeleted(val),
    ...
    fn seek_exact_skips_tombstone_returns_none() {
        ...
        assert!(
            out.is_none(),
            "seek_exact must filter out deleted latest value"
        );
    }
    ...
    fn account_seek_exact_live_maps_key_and_value() {
        ...
        let out = TrieCursor::seek_exact(&mut cur, k)
            .expect("ok")
            .expect("some");
        assert_eq!(out.0, k);
    }
    ...
    fn storage_seek_exact_respects_address_filter() {
        ...
        let out_b = TrieCursor::seek_exact(&mut cur_b, path)
            .expect("ok")
            .expect("some");
    ...
    fn storage_seek_returns_first_key_for_bound_address() {
        ...
        // test seek_exact behaviour
        {
            ...
            let out = TrieCursor::seek_exact(&mut cur_a, p2)
                .expect("ok")
                .expect("some");
            assert_eq!(out.0, p2);
    ...
    fn storage_next_stops_at_address_boundary() {
        ...
        // position at p1 (A)
        let _ = TrieCursor::seek_exact(&mut cur_a, p1)
            .expect("ok")
            .expect("some");
        ...
        assert!(
            out.is_none(),
            "next() should stop when next key is a different address"
        );
    }
    ...
    fn storage_current_maps_key() {
        ...
        let _ = TrieCursor::seek_exact(&mut cur, p)
            .expect("ok")
            .expect("some");
    ...
    fn storage_trie_cursor_next_without_seek_for_non_first_address() {
        ...
        assert!(
            result2.is_some(),
            "next() should work for non-first address without seek()"
        );
