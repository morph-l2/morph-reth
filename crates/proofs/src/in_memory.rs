//! In-memory implementation of [`MorphProofsStore`] for testing purposes
...
use crate::{
    BlockStateDiff, MorphProofsStorageError, MorphProofsStorageResult, MorphProofsStore,
    api::{
        InitialStateAnchor, InitialStateStatus, MorphProofsBatchSession, MorphProofsBatchStore,
...
impl InMemoryStorageInner {
    fn store_trie_updates(
        ...
        for (path, branch) in block_state_diff.sorted_trie_updates.account_nodes_ref() {
            self.account_branches
                .insert((block_number, *path), branch.clone());
            ...
            block_state_diff.sorted_trie_updates.storage_tries_ref()
        {
            ...
            for (path, branch) in storage_trie_updates.storage_nodes_ref() {
                self.storage_branches
                    .insert((block_number, *address, *path), branch.clone());
        ...
        for (address, account) in &block_state_diff.sorted_post_state.accounts {
            self.hashed_accounts
                .insert((block_number, *address), *account);
        ...
        for (hashed_address, storage) in &block_state_diff.sorted_post_state.storages {
            ...
            for (slot, value) in storage.storage_slots_ref() {
                self.hashed_storages
                    .insert((block_number, *hashed_address, *slot), *value);
                result.hashed_storages_written_total += 1;
            }
        }

        self.trie_updates
            .insert(block_number, block_state_diff.sorted_trie_updates.clone());
        self.post_states
            .insert(block_number, block_state_diff.sorted_post_state);
...
impl InMemoryProofsStorage {
    /// Create a new in-memory Morph proofs storage instance.
    pub fn new() -> Self {
        Self {
    ...
    fn get_latest_account_trie_key(&self) -> MorphProofsStorageResult<Option<StoredNibbles>> {
    ...
    fn get_latest_storage_trie_key(&self) -> MorphProofsStorageResult<Option<StorageTrieKey>> {
        let inner = self.inner.read();
        Ok(inner
            .storage_branches
            .range((
    ...
    fn get_latest_hashed_account_key(&self) -> MorphProofsStorageResult<Option<B256>> {
        let inner = self.inner.read();
        Ok(inner
            .hashed_accounts
            .range((
    ...
    fn get_latest_hashed_storage_key(&self) -> MorphProofsStorageResult<Option<HashedStorageKey>> {
        let inner = self.inner.read();
        Ok(inner
            .hashed_storages
            .range((
...
impl InMemoryTrieCursor {
    ...
    fn ensure_entries_populated(&mut self) -> Result<(), DatabaseError> {
        ...
        let storage = self
            .inner
            .try_read()
            .ok_or(MorphProofsStorageError::TryLockError)?;
...
impl MorphProofsStore for InMemoryProofsStorage {
    ...
    where
        ...
        'db: 'tx,
    {
        self.get_earliest_block_number()
    }
    ...
    fn fetch_trie_updates(&self, block_number: u64) -> MorphProofsStorageResult<BlockStateDiff> {
        ...
        let trie_updates = inner
            .trie_updates
            .get(&block_number)
            .cloned()
            .unwrap_or_default();
        let post_state = inner
            .post_states
            .get(&block_number)
            .cloned()
            .unwrap_or_default();
    ...
    fn unwind_history(&self, unwind_upto_block: BlockWithParent) -> MorphProofsStorageResult<()> {
        ...
        inner
            .trie_updates
            .retain(|block, _| *block <= unwind_upto_block_number);
        inner
            .post_states
            .retain(|block, _| *block <= unwind_upto_block_number);
        inner
            .account_branches
            .retain(|(block, _), _| *block <= unwind_upto_block_number);
        inner
            .storage_branches
            .retain(|(block, _, _), _| *block <= unwind_upto_block_number);
        inner
            .hashed_accounts
            .retain(|(block, _), _| *block <= unwind_upto_block_number);
        inner
    ...
    fn replace_updates(
        ...
        inner
            .trie_updates
            .retain(|block, _| *block <= latest_common_block_number);
        inner
            .post_states
            .retain(|block, _| *block <= latest_common_block_number);
        inner
            .account_branches
            .retain(|(block, _), _| *block <= latest_common_block_number);
        inner
            .storage_branches
            .retain(|(block, _, _), _| *block <= latest_common_block_number);
        inner
            .hashed_accounts
            .retain(|(block, _), _| *block <= latest_common_block_number);
        inner
...
impl MorphProofsBatchSession for InMemoryBatchSession<'_> {
    ...
    fn storage_trie_cursor(
        ...
        hashed_address: B256,
        max_block_number: u64,
    ) -> MorphProofsStorageResult<Self::StorageTrieCursor<'_>> {
        self.storage
            .storage_trie_cursor(hashed_address, max_block_number)
    }

    fn account_trie_cursor(
        &self,
        max_block_number: u64,
    ) -> MorphProofsStorageResult<Self::AccountTrieCursor<'_>> {
    ...
    fn storage_hashed_cursor(
        ...
        hashed_address: B256,
        max_block_number: u64,
    ) -> MorphProofsStorageResult<Self::StorageCursor<'_>> {
        self.storage
    ...
    fn account_hashed_cursor(
        &self,
        max_block_number: u64,
    ) -> MorphProofsStorageResult<Self::AccountHashedCursor<'_>> {
...
impl MorphProofsBatchStore for InMemoryProofsStorage {
    type BatchSession<'a>
...
impl MorphProofsInitialStateStore for InMemoryProofsStorage {
    ...
    fn store_storage_branches(
        ...
        for (path, branch) in items {
            inner
                .storage_branches
                .insert((0, hashed_address, path), branch);
        }
    ...
    fn store_hashed_storages(
        ...
        for (slot, value) in storages {
            inner
                .hashed_storages
                .insert((0, hashed_address, slot), value);
        }
    ...
    fn commit_initial_state(&self) -> MorphProofsStorageResult<BlockNumHash> {
        ...
        if let Some((number, hash)) = inner.anchor_block {
            ...
            Err(MorphProofsStorageError::NoBlocksFound)
        }
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::MorphProofsStorageError;

    #[test]
    fn test_in_memory_storage_basic_operations() -> Result<(), MorphProofsStorageError> {
    ...
    #[test]
    fn test_trie_updates_storage() -> Result<(), MorphProofsStorageError> {
        let storage = InMemoryProofsStorage::new();
