...
//! 2. Verifies `eth_balance >= value` (and for `fee_token_id == 0`, ETH covers full cost)
//! 3. Checks the token is registered and active in L2TokenRegistry, with a non-zero price ratio
//! 4. Calculates required token amount:
//!    `TokenFeeInfo::eth_to_token_amount(gas_limit * max_fee_per_gas + l1_data_fee)`
//! 5. Uses effective token limit semantics:
//!    - `fee_limit == 0` => treat as token balance
//!    - `fee_limit > balance` => cap by token balance
//! 6. Verifies effective limit can cover required token amount
